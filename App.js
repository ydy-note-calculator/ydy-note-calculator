...
export default function App() {
  ...
  const calculateGrade = () => {
    ...
    if (ort >= limit) {
      ...
      res.fH = fS; 
      if (fS >= 64.5) { 
        res.durum = 'Finalle Geçtiniz ✓'; 
        res.renk = theme.accent; 
        setTargetNote(null); 
      } else {
        res.durum = 'Bütünlemeye Kaldınız'; 
        res.renk = '#ef4444';
        if (needed <= 100) { 
      ...
      res.renk = isP ? theme.accent : '#ef4444';
      setTargetNote(null);
    }
  ...
  const shareOnWhatsApp = () => {
    ...
    if (typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'whatsapp_paylasimi', { 'event_category': 'Sosyal', 'event_label': 'WhatsApp Paylaşımı Yapıldı' });
    }
  };
      ...
      <View style={styles.flexItem}>
        ...
        <TextInput 
          ...
          onChangeText={t => {
            ...
            if (v !== '' && parseInt(v) > 100) {
              if (typeof window !== 'undefined' && window.gtag) {
                window.gtag('event', 'hatali_giris', { 'event_category': 'Hata', 'event_label': `Hatalı Not (>100): ${label}` });
              }
              return; 
            }
